...
export type Tpalette<T, A> = {
	// mix: TmixFunction<T, A>, 
	createMix: (themeIndex?: number) => <Props = null, Propless = null>(arg: TmixArgs<Props, Propless, T, A>) => (props?: Partial<Propless extends null ? Props : { [Key in keyof Propless]: Propless[Key] extends object ? keyof Propless[Key] : boolean; }>) => string
